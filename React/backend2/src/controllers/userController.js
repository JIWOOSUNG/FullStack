...
exports.duplicatedEmail = async (req, res) => {
    ...
    if(!email) {
        return res.status(400).json( { message: '이메일을 입력하세요'}); //400은 클라이언트쪽 에러
    }
    try {
        ...
        console.log('result:', result);
        // res.json(result);
        // 해당 email이 없다면 빈배열 [] 을 반환, 있으면 [{id: 회원번호}]
        if(result.length === 0){ //해당 이메일 사용 가능
...
exports.deleteUser = async(req, res) => {
    try {
        const {id} =req.params;
        if(!id) {
            return res.status(400).json({message:'회원번호가 필요합니다'});
        }
...
exports.updateUser = async(req, res) => {
    ...
    try {
        const data = [name, email, passwd, id];
        const sql = `update members set name=?, email=?, passwd=? where id=?`;
