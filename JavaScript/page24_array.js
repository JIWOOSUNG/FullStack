//배열의 주요 인스턴스 매서드 (함수)
let scores = [55,89,99,68,77];
//배열.join('구분자') => 배열 요소들을 구분자로 결합하여 문자열로 반환
let str = scores.join('@');
console.log(str);
//split('구문자')=> 문자열을 구분자로 분리하여 배열로 반환
...
//slice() : 배열의 일부를 가져올 떄 사용. 원본 배열을 유지함
...
// scores 에서 99빼고 그 자리에 90을 추가
let arr2 = scores.splice(2,1,90); // 원본 배열을 직접 변경함
console.log(arr2); //삭제한 요소를 배열로 반환
console.log(scores); // [ 55, 89, 90, 68, 77 ]

scores.splice(2,2);
console.log(scores); // [ 55, 89, 77 ]
//push() unshift()
...
//indexOf(값) : 배열에 값이 있는지 검색해서 있으면 해당 인덱스 번호 반환
//              없으면 -1을 반환
...
//Array.from() : 유사배열 객체를 배열로 변환하는 정적(static) 매서드
